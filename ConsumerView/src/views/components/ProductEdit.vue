...
import {productAddService, productGetByIdService, productUpdateService} from "@/api/product";
...
const onSubmit = async () => {
  ...
  if (formModel.value.id) {
    console.log("Edit")
    const res = await productUpdateService(fd)
    if (res.code === 1) {
  ...
  <el-drawer v-model="visibleDrawer" :title="formModel.id?'编辑商品':'添加商品'" size="50%">
    <el-form :model="formModel" ref="formRef" label-width="100px">
      ...
      <el-form-item label="商品类型" prop="cate_id">
        <el-select v-model="formModel.cate_id" placeholder="请选择商品类型" disabled>
